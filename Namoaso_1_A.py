...
class FingerGameScreen_1_A(Screen):
    ...
    def ai_turn(self):
        """ AIのターン """
        state = self.env.get_state()  # 現在の状態を取得
        action = self.agent.choose_action(state)  # AIが行動を選択
        ...
        # ① AIが選んだ手をUIに反映
        self.on_image_tap_1(player_hand)  # AIの手を選択状態にする
        self.on_image_tap_2(opponent_hand)  # 対象の手を選択状態にする

        # ② ゲームの1ターンを進める
        next_state, reward, done, _ = self.env.step(action)  # AIの手を適用
        self.agent.update_q(state, action, reward, next_state)  # Qテーブルを更新
        self.agent.save_q_table()  # 学習データを保存
        ...
        if done:
            ...
            Clock.schedule_once(lambda dt: self.next_turn(), 0.5)  # 0.5秒後に次のターン
